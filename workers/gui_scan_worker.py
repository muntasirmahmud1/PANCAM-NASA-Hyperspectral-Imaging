...
class GuiAzScanWorker(QObject):
    ...
    # ----------------------------------------------------
    # Exposure/filter optimization with edge-stop safeguards
    # ----------------------------------------------------

    def _optimize_filter_and_exposure(
        self,
        *,
        start_filter,
        start_exposure_ms,
        mode_text,
        max_iters_per_filter=20,
    ):
        if start_filter not in self.filter_sequence:
            start_filter = self.filter_sequence[0]

        filter_idx = self.filter_sequence.index(start_filter)
        current_exp = max(self.exposure_min_ms, min(self.exposure_max_ms, float(start_exposure_ms)))

        best_state = None

        while 0 <= filter_idx < len(self.filter_sequence):
            filt = self.filter_sequence[filter_idx]
            self._set_filter(filt)
            self._set_exposure(current_exp)

            for it in range(max_iters_per_filter):
                if self.stop_requested:
                    return filt, self.exposure_ms, {}

                img = self._capture_average(f"{mode_text} F{filt} iter {it + 1}")
                m = self._metrics(img)
                best_state = (filt, self.exposure_ms, m)

                self.log.emit(
                    f"[OPT] {mode_text} | F{filt} exp={self.exposure_ms:.2f} ms "
                    f"ctrl={m['control_dn']:.1f} sat={m['saturated']} "
                    f"too_dim={m['too_dim']} too_bright={m['too_bright']} good={m['good']}"
                )

                if m["good"]:
                    return filt, self.exposure_ms, m

                brightest_filter_idx = 0
                darkest_filter_idx = len(self.filter_sequence) - 1

                at_brightest_possible = (
                    filter_idx == brightest_filter_idx
                    and self.exposure_ms >= self.exposure_max_ms - 1e-6
                )

                at_darkest_possible = (
                    filter_idx == darkest_filter_idx
                    and self.exposure_ms <= self.exposure_min_ms + 1e-6
                )

                # Cloudy/daytime protection:
                # If open/brightest filter + max exposure is still too dim,
                # stop trying and accept current state.
                if m["too_dim"] and at_brightest_possible:
                    self.log.emit(
                        "[OPT-STOP] Still too dim at brightest filter and max exposure. "
                        "Accepting current setting to avoid long optimization."
                    )
                    return filt, self.exposure_ms, m

                # Sun protection:
                # If strongest ND/darkest filter + min exposure is still saturated,
                # stop trying and accept current state.
                if (m["saturated"] or m["too_bright"]) and at_darkest_possible:
                    self.log.emit(
                        "[OPT-STOP] Still saturated/too bright at darkest filter and min exposure. "
                        "Accepting current setting."
                    )
                    return filt, self.exposure_ms, m

                if m["too_dim"]:
                    target = 0.65 * self.max_dn
                    if m["control_dn"] > 0:
                        ratio = target / float(m["control_dn"])
                        ratio = max(1.15, min(ratio, 1.8))
                    else:
                        ratio = 1.8

                    new_exp = min(self.exposure_max_ms, self.exposure_ms * ratio)

                    if new_exp >= self.exposure_max_ms - 1e-6:
                        self._set_exposure(self.exposure_max_ms)

                        if filter_idx > brightest_filter_idx:
                            filter_idx -= 1
                            current_exp = self.exposure_ms
                            break

                        # Already brightest filter
                        continue

                    self._set_exposure(new_exp)
                    continue

                if m["saturated"] or m["too_bright"]:
                    target = 0.85 * self.max_dn
                    if m["control_dn"] > 0:
                        ratio = target / float(m["control_dn"])
                        ratio = max(0.35, min(ratio, 0.95))
                    else:
                        ratio = 0.5

                    new_exp = max(self.exposure_min_ms, self.exposure_ms * ratio)

                    if new_exp <= self.exposure_min_ms + 1e-6:
                        self._set_exposure(self.exposure_min_ms)

                        if filter_idx < darkest_filter_idx:
                            filter_idx += 1
                            current_exp = self.exposure_ms
                            break

                        # Already darkest filter
                        continue
                ...
                # max iterations reached for this filter
                self.log.emit(f"[OPT-STOP] Max optimization iterations reached for filter {filt}.")
                return best_state

        if best_state is not None:
            return best_state

        raise RuntimeError("Exposure/filter optimization failed.")
